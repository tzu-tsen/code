...
dataset = Dataset.Dataset(root_path=p.path_test,transform_train=p.transform_train, transform_test=p.transform_val,class_number=p.num_class, roi_transform_train=p.roi_transform_train_no_padding,roi_transform_test=p.roi_transform_val_no_padding, data_type=parameter.EnumType.roi)
...
for i in range(10):
    ...
    test_loader = torch.utils.data.DataLoader(dataset, batch_size=p.batch_size_roi,sampler=test_sampler,num_workers=p.num_workers)    
    ...
    for epoch in range(p.num_epochs):
        ...
        test_acc, test_loss, test_target_lists, test_predict_lists = fun.test(test_loader, model, criterion, hyperparameter=p.beta_roi)
