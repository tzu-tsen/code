...
def train(train_loader, model, criterion, optimizer, hyperparameter, num_class=8):
    ...
    for step, all_data in enumerate(train_loader):
        ...
        target, data, eye_data, nose_data, mouth_data= target.cuda(), datas['train'].cuda(), eye_datas['train'].cuda(), nose_datas['train'].cuda(), mouth_datas['train'].cuda()
...
def test(val_loader, model, criterion, hyperparameter):
    with torch.no_grad():        
        running_loss = 0.0
        iter_cnt = 0
        bingo_cnt = 0
