...
class Parameter:
    ...
    def __init__(self,          
        ...
        lr_roi=0.1,
        lr_global=0.1,
        milestones_roi = [10,20,40],
        milestones_global = [5,10,15,20,25,30,35,40,45,50,55],
        gamma_roi = 0.1,
        gamma_global = 0.1,
        weight_decay=0.0001, 
        momentum = 0.9,
        num_class=8, 
        num_class_samples_roi=10000,
        num_class_samples_global=500,
        database="AffectNet",
        cm_normalize = True,
        beta_roi = [1, 0.6, 1],
        size=224,
        ...
        self.batch_size_global = batch_size_global
        ...
        self.milestones_global = milestones_global
        # 每次下降多少
        ...
        self.num_samples_global = num_class_samples_global*num_class
        ...
        self.roi_transform_train_no_padding_randomerasing03 = transforms.Compose(
        [
            transforms.Resize((self.roi_size,self.roi_size)), #90*90
            transforms.RandomHorizontalFlip(),
            #亮度（brightness）、對比度（contrast）、飽和度（saturation）和色调（hue)
        ...
        self.roi_transform_train_no_padding_no_randomerasing = transforms.Compose(
        [
            transforms.Resize((self.roi_size,self.roi_size)), #90*90
            transforms.RandomHorizontalFlip(),
            #亮度（brightness）、對比度（contrast）、飽和度（saturation）和色调（hue)
