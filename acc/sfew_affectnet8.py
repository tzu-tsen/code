...
# %% [markdown]
#  ### 會調整的參數設定
p = parameter.Parameter(database="SFEW2.0",num_class_end=7)
...
# best_model_roi = model1.ResNet(num_classes=p.num_class, n_head=p.n_head, resnet_cbam=p.num_resnet_cbam).cuda() # we do not specify pretrained=True, i.e. do not load default weights
# best_model_roi.load_state_dict(torch.load(path_model_roi))
...
val_figure_cm = ConfusionMatrix.figure_confusion_matrix(
    val_target_lists, val_predict_lists, p.class_names, True)
writer.add_figure('Confusion Matrix val', val_figure_cm, 1)

val_figure_cm = ConfusionMatrix.figure_confusion_matrix(
    val_target_lists, val_predict_lists, p.class_names, False)
writer.add_figure('Confusion Matrix val', val_figure_cm, 2)
