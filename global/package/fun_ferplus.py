import torch
import numpy as np

# def binary_accuracy(preds, y, logits=False):
#     if logits:
#         rounded_preds = torch.round(torch.sigmoid(preds))
#     else:
#         rounded_preds = torch.round(preds)
#     correct = (rounded_preds == y).float()
#     accuracy = correct.sum() / len(y)
#     return accuracy

def correct(predicts, target):    
    total = 0
    index = 0
    for i in target:
        j = np.where(i>0)[0]
        
        p = predicts[index].item()
        k = np.where(j==p)[0]
        total = total +1 if len(k)>0 else total
        index += 1
      
    return total

def train_no_roi(train_loader, model, criterion, optimizer, num_class=8):
    loss_sum = 0.0
    correct_sum = 0
    iter_cnt = 0
    target_lists, predict_lists = [], []    
    
    # train_loss = 0.0
    sample_cnt=0

    # switch to train mode
    model.train()
    for step, all_data in enumerate(train_loader):
        iter_cnt += 1              

        # 梯度清零
        optimizer.zero_grad()
        
        target, data = all_data        
        target_lists+=target
        target, data = target.cuda(), data.cuda()
            
        output = model(data)           
        loss = criterion(output, target)
        
        # backward pass: compute gradient of the loss with respect to model parameters
        loss.backward()
        # 執行單個優化步驟（參數更新）
        optimizer.step()
        
        loss_sum += loss        
        _, predicts = torch.max(output, 1)  
        # # # predicts_lists.append(predicts)
        # # correct_sum += torch.eq(predicts,target).sum().cpu()
        # # # train_loss += loss.item()*output.size(0)
        
        correct_sum += correct(predicts.cpu(), target.cpu())

        sample_cnt += output.size(0)

        # 將preds預測結果detach出來，並轉成numpy格式               
        predict_lists+=predicts.view(-1).detach().cpu().numpy().tolist()
        
    acc = correct_sum / float(sample_cnt)    
    acc = np.around(acc,4)
    
    running_loss = loss_sum/iter_cnt
    running_loss = running_loss.cpu().detach().numpy()


    # target_lists_number = torch.sum(torch.FloatTensor(target_lists), dim=0)
    target_lists_number = []
       
    return acc, running_loss, target_lists_number, target_lists, predict_lists


def validate_no_roi(val_loader, model, criterion):
    with torch.no_grad():        
        running_loss = 0.0
        iter_cnt = 0
        bingo_cnt = 0
        sample_cnt = 0
        target_lists, predict_lists = [], []
            
        # switch to evaluate mode
        model.eval()
        for step, all_data in enumerate(val_loader):
            target, data = all_data
            target_lists+=target.tolist()
            target, data = target.cuda(), data.cuda()

            # forward pass: compute predicted outputs by passing inputs to the model
            output = model(data)      

            # update average validation loss 
            loss = criterion(output, target)

            # measure accuracy and record loss
            running_loss += loss
            iter_cnt+=1
            _, predicts = torch.max(output, 1)
            # correct_num  = torch.eq(predicts,target)
            # bingo_cnt += correct_num.sum().cpu()
            bingo_cnt += correct(predicts.cpu(), target.cpu())            
            sample_cnt += output.size(0)

            # 將preds預測結果detach出來，並轉成numpy格式               
            predict_lists+=predicts.view(-1).detach().cpu().numpy().tolist()        
        
        running_loss = running_loss/iter_cnt 
        acc = bingo_cnt/float(sample_cnt)
        acc = np.around(acc,4)

    return acc, running_loss, target_lists, predict_lists


    
def test_no_roi(test_loader, criterion, path_model):    
    with torch.no_grad(): 
        running_loss = 0.0
        iter_cnt = 0
        bingo_cnt = 0
        sample_cnt = 0
        target_lists, predict_lists = [], []
        
        model = torch.load(path_model)    
        model.cuda()        
        
        model.eval()
        for step, all_data in enumerate(test_loader):
            target, data = all_data
            target_lists+=target.tolist()
            target, data = target.cuda(), data.cuda()

            # forward pass: compute predicted outputs by passing inputs to the model
            output = model(data)      

            # update average validation loss 
            loss = criterion(output, target)

            # measure accuracy and record loss
            running_loss += loss
            iter_cnt+=1
            _, predicts = torch.max(output, 1)
            # correct_num  = torch.eq(predicts,target)
            # bingo_cnt += correct_num.sum().cpu()
            bingo_cnt += correct(predicts.cpu(), target.cpu())            
            sample_cnt += output.size(0)

            # 將preds預測結果detach出來，並轉成numpy格式               
            predict_lists+=predicts.view(-1).detach().cpu().numpy().tolist()            

        running_loss = running_loss/iter_cnt 
        acc = bingo_cnt/float(sample_cnt)
        acc = np.around(acc,4)

    return acc, running_loss, target_lists, predict_lists 
