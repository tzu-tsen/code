...
import dataset.FERPlus_no_roi as Dataset
...
# %%
train_dataset = Dataset.Dataset(root_path=p.path,run=parameter.EnumRun.train, transform=p.transform_train, data_type=parameter.EnumType.original)
...
test_dataset = Dataset.Dataset(root_path=p.path, run=parameter.EnumRun.test, transform=p.transform_val, 
                              data_type=parameter.EnumType.original)
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=p.batch_size_whole,shuffle=p.shuffle,num_workers=p.num_workers)    
...
for epoch in range(p.num_epochs):
    ...
    # train for one epoch    
    train_acc, train_loss, train_class_number, train_target_lists, train_predict_lists = fun.train_no_roi(train_loader, model, criterion, optimizer, num_class=p.num_class)
    
    # evaluate on validation set                       
    val_acc, val_loss, val_target_lists, val_predict_lists = fun.validate_no_roi(val_loader, model, criterion)
...
# 測試 model
best_acc_test, best_loss_test, test_target_lists, test_predict_lists = fun.test_no_roi(test_loader, criterion, path_model)
# test_figure_cm = ConfusionMatrix.figure_confusion_matrix(test_target_lists, test_predict_lists, p.class_names, p.cm_normalize) 
# writer.add_figure('Confusion Matrix test', test_figure_cm)
